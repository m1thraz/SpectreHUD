...
def test_settings_apply_updates_running_ui_font_without_restart(qapp):
...
def test_settings_apply_updates_running_code_font_without_restart(qapp):
    controller = _runtime_style_harness()
    previous_stylesheet = qapp.styleSheet()
    try:
...
def test_font_apply_does_not_activate_pending_theme_before_restart(qapp):
    controller = _runtime_style_harness()
    previous_stylesheet = qapp.styleSheet()
    try:
...
def test_report_font_remains_independent_from_application_fonts(qapp):
    controller = _runtime_style_harness()
    previous_stylesheet = qapp.styleSheet()
    try:

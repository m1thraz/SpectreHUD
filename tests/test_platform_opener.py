...
def test_open_path_opens_existing_file_with_local_qt_url(tmp_path):
    ...
    with patch("core.platform.opener.QDesktopServices.openUrl", return_value=True) as opener:
...
def test_open_path_rejects_missing_path_without_desktop_call(tmp_path):
    ...
    with patch("core.platform.opener.QDesktopServices.openUrl") as opener:
...
def test_open_path_reports_qt_desktop_failure(tmp_path):
    ...
    with patch("core.platform.opener.QDesktopServices.openUrl", return_value=False):
        assert open_path(target) is False
...
def test_open_path_contains_qt_runtime_failure(tmp_path):
    ...
    with patch(
        "core.platform.opener.QDesktopServices.openUrl",

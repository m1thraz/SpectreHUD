...
The cross-platform test runner also provides `full`, `release`, `all`,
`targeted`, and `last-failed` modes. Add `--json` for a single machine-readable
result intended for agents. Calling it without a mode runs the complete
unfiltered suite.
